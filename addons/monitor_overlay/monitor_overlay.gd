...
#	if vertices_drawn:
#		_create_graph_for(Performance.render, "Vertices Drawn")
...
#		_create_graph_for(Performance.RENDER_2D_DRAW_CALLS_IN_FRAME, "2D Draw calls")
	...
	if texture_memory:
		_create_graph_for(Performance.RENDER_TEXTURE_MEM_USED, "Texture2D Memory", "B")
...
func _process(_delta) -> void:
	for item in _graphs:
		# Silly hack to cause repaint since update() was removed from 4.0
		item.visible = false
		item.visible = true
